...
def obtenir_prediccions_grups(df_j):
    files = []
    for g in list("ABCDEFGHIJKL"):
        files.append({
            "Grup": f"Grup {g}",
            "1r": afegir_bandera(valor_o_pendent(df_j[f"Grup {g} 1r"].values[0])) if f"Grup {g} 1r" in df_j.columns else "Pendent",
            "2n": afegir_bandera(valor_o_pendent(df_j[f"Grup {g} 2n"].values[0])) if f"Grup {g} 2n" in df_j.columns else "Pendent",
            "3r": afegir_bandera(valor_o_pendent(df_j[f"Grup {g} 3r"].values[0])) if f"Grup {g} 3r" in df_j.columns else "Pendent",
        })
...
def mostrar_prediccions_participant(df_j):
    st.write("### 📋 Prediccions de grups")
    st.dataframe(obtenir_prediccions_grups(df_j), use_container_width=True, hide_index=True)
...
.header-card {{ height: 210px; min-height: 210px; }}
.result-card {{ height: 190px; min-height: 190px; }}
...
with st.sidebar:
    ...
    if st.button("🧹 Reiniciar històric d’evolució"):
        ...
        st.rerun()
    st.markdown("---")
    st.caption("Mode estable: Excel, sense API externa.")
...
st.markdown('<p class="subtitle">Classificació en viu, resultats Excel, competició per departaments i evolució temporal</p>', unsafe_allow_html=True)
...
if jugador is not None:
    ...
    if not df_j.empty:
        ...
        mostrar_prediccions_participant(df_j)
else:
...
if gols_pichichi != "Pendent":
    pichichi_text = f"{pichichi_real} ({gols_pichichi})"
else:
    pichichi_text = pichichi_real
...
st.markdown("---")
st.info("Mode estable: Excel. L’app no utilitza API externa.")
st.write("📡 Actualització automàtica des d’Excel")
